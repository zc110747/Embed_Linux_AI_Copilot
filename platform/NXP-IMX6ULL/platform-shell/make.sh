...
FILE_CROSS_COMPILE="${CROSS_COMPILE}"
THREAD_COUNT=3
...
KERNEL_PATH="${AGENT_SDK_SOC_PATH}/kernel"
ROOTFS_PATH="${AGENT_SDK_SOC_PATH}/rootfs"
PACKAGE_PATH="${AGENT_SDK_SOC_PATH}/package"
ROOTFS_SHELL_PATH="${AGENT_PLATFORM_SOC_PATH}/platform-shell"

KERNEL_ZIMAGE="${KERNEL_PATH}/arch/arm/boot/${ZIMAGE_FILE}"
KERNEL_DTB="${KERNEL_PATH}/arch/arm/boot/dts/${DTB_EMMC_FILE}"
KERNEL_NAND_DTB="${KERNEL_PATH}/arch/arm/boot/dts/${DTB_NAND_FILE}"
KERNEL_QEMU_DTB="${KERNEL_PATH}/arch/arm/boot/dts/${QEMU_DTB_FILE}"
...
compile_debian()
{
    ...
    chmod 777 install-debian.sh && ./install-debian.sh
